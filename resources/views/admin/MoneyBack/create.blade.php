...
<style>
    ...
    .cap-badge{background:#eef7f1;color:#116631;border:1px solid #cfe9db;padding:2px 8px;border-radius:999px;font-size:.8rem}
</style>
@endsection
...
<div class="main-content pt-0">
    ...
    @if ($booking)
        @php
            // Safe server-side totals
            $deposit     = (float) ($booking->deposit_amount ?? 0);
            // Prefer controller-provided sum → booking cached column → DB sum
            $itemsTotal  = isset($booking->items_total) ? (float)$booking->items_total
                         : ((isset($booking->items_amount) && $booking->items_amount !== null)
                            ? (float)$booking->items_amount
                            : (float) $booking->items()->sum('total'));
            $refundable  = max($deposit - $itemsTotal, 0); // Deposit minus items (no negatives)
            // Respect controller's $maxAmount if it was passed; otherwise use computed
            $maxShown    = isset($maxAmount) ? (float)$maxAmount : $refundable;
        @endphp
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="stat">
                                ...
                                <span class="ms-2 text-muted small">(Deposit − Items)</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <form method="POST" action="{{ route('admin.money-back.store') }}" class="mbk-card">
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-12 col-md-6">
                <label class="form-label d-flex align-items-center gap-2">
                    Amount (₹)
                    <span class="badge bg-light text-dark border">Max: ₹{{ number_format($maxShown, 2) }}</span>
                </label>
                <input
                    type="number"
                    step="0.01"
                    min="0.01"
                    max="{{ number_format($maxShown, 2, '.', '') }}"
                    name="amount"
                    id="amountInput"
                    value="{{ old('amount', number_format($maxShown, 2, '.', '')) }}"
                    class="form-control @error('amount') is-invalid @enderror"
                    placeholder="0.00">
                @error('amount') <div class="invalid-feedback">{{ $message }}</div> @enderror

                @if($maxShown <= 0)
                    <div class="form-text text-danger">No refundable balance remains for this booking.</div>
                @endif
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="note" rows="3" class="form-control @error('note') is-invalid @enderror" placeholder="Internal note…">{{ old('note') }}</textarea>
            @error('note') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-success" @if($maxShown<=0) disabled @endif>
...
(function(){
  ...
  amount.addEventListener('input', () => {
    let v = parseFloat(amount.value || '0');
