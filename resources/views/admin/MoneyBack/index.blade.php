...
<style>
    ...
    .custom-input-icon input{padding-left:2rem}
    .custom-input-icon .material-icons{
        position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#aaa;font-size:1.2rem
    }
    .badge-refund{background:#ffe5e8;color:#b4232c;border:1px solid #ffccd1}
    .badge-return{background:#eef7f1;color:#116631;border:1px solid #cfe9db}
        ...
        <form method="GET" action="{{ route('admin.money-back.index') }}" class="d-flex flex-wrap gap-2 mb-4">
            ...
            <div class="position-relative custom-input-icon">
                <input
                    ...
                    class="form-control"
                    style="width:230px"
                    placeholder="Search (ID / amount / user / venue / slot)…">
            </div>
            ...
            <tbody>
                @forelse($moneyBacks as $moneyBack)
                    @php
                        ...
                        $venueName = $moneyBack->booking && $moneyBack->booking->venue
                            ? $moneyBack->booking->venue->name : '—';

                        $slotName  = $moneyBack->booking && $moneyBack->booking->timeSlot
